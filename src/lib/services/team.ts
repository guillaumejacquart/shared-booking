...
export async function createOffice(
  ...
  const slugTaken = await store.getOfficeBySlug(deps.db, input.slug);
  ...
  for (let n = 2; ; n++) {
    const taken = await store.getPractitionerBySlug(deps.db, practitionerSlug);
    if (!taken) break;
    practitionerSlug = `${base}-${n}`;
  }

  const officeId = crypto.randomUUID();
  await store.createOffice(deps.db, {
    office: {
      id: officeId,
      name: input.name,
      slug: input.slug,
...
export async function createInvite(
  ...
  const requester = await store.getMembership(deps.db, input.officeId, input.requesterUserId);
  ...
  const existingUser = await store.getUserByEmail(deps.db, email);
  if (existingUser) {
    const existing = await store.getMembership(deps.db, input.officeId, existingUser.id);
    if (existing) throw new ConflictError("Cette personne est déjà membre du cabinet");
  }

  const token = randomBytes(32).toString("hex");
  const id = await store.createInvite(deps.db, {
    id: crypto.randomUUID(),
...
export async function acceptInvite(
  ...
  const inv = await store.getInviteByToken(deps.db, input.token);
  ...
  const office = await store.getOfficeById(deps.db, inv.officeId);
  if (!office) throw new NotFoundError("Cabinet introuvable");

  const already = await store.getMembership(deps.db, inv.officeId, input.userId);
  if (already) throw new ConflictError("Vous êtes déjà membre de ce cabinet");

  const base = slugify(input.userName);
  let slug = base;
  for (let n = 2; ; n++) {
    const taken = await store.getPractitionerBySlug(deps.db, slug);
    if (!taken) break;
    slug = `${base}-${n}`;
  }

  await store.acceptInvite(deps.db, {
    inviteId: inv.id,
    now,
    member: {
      id: crypto.randomUUID(),
